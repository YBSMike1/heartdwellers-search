...
<style>
    ...
    @media (prefers-color-scheme: dark) {
        ...
        h1, h2, h3, .stMarkdown, label, .stTextInput label, .stTextInput input, .stText, .stSpinner, .stProgress label, .stEmpty, .stSuccess, .stInfo, .stWarning, .stError, div[data-testid="stText"] {
...
def search_italic_text(search_word, folder_path):
    ...
    for root, dirs, files in os.walk(folder_path):
        for filename in files:
            if filename.lower().endswith('.docx') and not any(skip in filename.lower() for skip in ["compilation ", "~$", "eom", "all messages"]):
    ...
    with ThreadPoolExecutor(max_workers=12) as executor:
        ...
        for i, future in enumerate(as_completed(future_to_file)):
            ...
            if result:
                ...
                match_count += 1
            file_count += 1
            progress = min(int((i + 1) / max(total_files, 1) * 100), 100)
        ...
        if results:
            ...
            for res in results:
                ...
                with st.expander(f"📄 {res['file']}", expanded=True):
                    st.markdown(f"""
                    <div style="font-family: Calibri, Arial, sans-serif; font-size: 0.92em; line-height: 1.75; background-color: #FFCCE0; padding: 18px; border-radius: 10px; border-left: 6px solid #D81B60; color: #1e1e2e;">
                        {highlighted}
                    </div>
                    """, unsafe_allow_html=True)
            # Download
